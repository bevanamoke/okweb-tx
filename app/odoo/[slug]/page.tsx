...
export function generateStaticParams() {
    return odooServices.map((service) => ({
        slug: service.slug,
    }))
}

export function generateMetadata({ params }: { params: { slug: string } }) {
    const service = odooServices.find((s) => s.slug === params.slug)
    if (!service) return { title: "Service Not Found" }

    return {
        title: `${service.title} | OKS Odoo Services`,
...
export default function ServicePage({ params }: { params: { slug: string } }) {
    ...
    return (
        <main className="min-h-screen bg-background pt-24 pb-16">
                    ...
                    <div className="relative h-[400px] rounded-3xl bg-card border border-border p-8 overflow-hidden group">
                        ...
                        <div className="relative z-10 h-full flex flex-col justify-between">
                            ...
                            <div className="grid gap-4">
                                {service.content.benefits.slice(0, 3).map((benefit, i) => (
                ...
                {service.content.faq && (
                    <div className="mb-20">
                        ...
                        <div className="space-y-4">
                            {service.content.faq.map((item: any, i: number) => (
            ...
            {/* Related Services */}
            <section className="container mx-auto px-4 max-w-7xl mb-24 border-t border-border pt-20">
                <h2 className="text-3xl font-bold mb-12 text-center">Explore Other Odoo Services</h2>
                <div className="grid md:grid-cols-3 gap-6">
                    {odooServices.filter(s => s.slug !== params.slug).slice(0, 3).map((related) => (
                        <Link key={related.id} href={`/odoo/${related.slug}/`} className="group p-8 rounded-2xl bg-card border border-border hover:border-primary/50 hover:shadow-lg transition-all">
                            <related.icon className="w-10 h-10 text-primary mb-4 group-hover:scale-110 transition-transform" />
                            <h3 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors">{related.title}</h3>
                            <p className="text-sm text-foreground/60">{related.description}</p>
                        </Link>
                    ))}
                </div>
            </section>
